...
def execute(infile, outfile, extras):
    ...
    commands = {
        ...
        'd8-wasm':
        [runner, '--experimental-wasm-bigint', '--no-wasm-async-compilation'] +
        wasmjs + ['--', infile] + extra_files,
        'd8-asm2wasm': [
            runner, '--experimental-wasm-bigint',
            '--no-wasm-async-compilation', infile
        ],
        'jsc-wasm':
        [runner, '--useWebAssembly=1'] + wasmjs + ['--', infile] + extra_files,
